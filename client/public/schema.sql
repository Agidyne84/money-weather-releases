...
-- Transaction rules table (for import auto-assignment)
CREATE TABLE IF NOT EXISTS transaction_rules (
    id TEXT PRIMARY KEY,
    transaction_id TEXT,                              -- budget item (primary target; nullable if item deleted)
    account_id TEXT,                                  -- originating account (advisory; null = any account)
    restrict_to_account INTEGER NOT NULL DEFAULT 0,   -- 1 = only match rows from account_id
    pattern TEXT NOT NULL,                            -- normalised description substring to match
    category_id TEXT NOT NULL,                        -- fallback category if transaction_id is deleted
...
CREATE TABLE IF NOT EXISTS historical_transactions (
    ...
    transfer_to_account_id TEXT,
    is_transfer INTEGER DEFAULT 0,
    -- 1 when a row is excluded from Dashboard/Forecast analyses but kept in the DB.
    ...
    -- 1 when this row has been committed from a bank import.
    is_posted INTEGER NOT NULL DEFAULT 1,
    -- Original CSV description preserved for rule creation and audit.
